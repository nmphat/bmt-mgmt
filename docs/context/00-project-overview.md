...
| i18n         | Custom i18n (`messages.ts` + `lang` store), hỗ trợ `vi` và `en` |
...
### snapshot.status (Postgres enum `payment_status`)
...
| 🟡 DEBT   | Cần tiếp tục đồng bộ docs/API snapshot với DB thực tế             |
| 🟢 DEBT   | Cần cleanup một số chỗ dùng `isAuthenticated` thay vì `isAdmin` ở UI |
