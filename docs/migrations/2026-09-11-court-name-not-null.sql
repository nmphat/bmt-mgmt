-- session_court_bookings.court_name: close the reachable-NULL hole.
-- Run once, after 2026-09-09-phase1-pricing.sql.
--
-- Reviewed against docs/sql-export/{02_tables,06_functions}.sql as of
-- commit 822e8c2. Production measurement, 2026-09-11: 47 booking rows, 0
-- with a NULL court_name -- the constraint applies cleanly. The function
-- body below is copied verbatim from 06_functions.sql after adding the
-- input guard; no other function is touched.
--
-- Pre-flight (run first, expect 0):
--   SELECT count(*) FROM session_court_bookings
--   WHERE court_name IS NULL OR trim(court_name) = '';

BEGIN;

-- ALTER COLUMN ... SET NOT NULL takes ACCESS EXCLUSIVE on
-- session_court_bookings and holds it to COMMIT, same reason as the phase 1
-- migration's lock_timeout. Run in a quiet window regardless.
